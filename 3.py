...
class OpenRouterClient:
    ...
    def create_sources_table(self, results: List[Dict]) -> str:
        """Create a nicely formatted sources table"""
        if not results:
            return "\n\n## 📚 No sources found"
        
        table = "\n\n## 📚 Sources Used:\n\n"
        table += "| # | Source File | Relevance Score | Content Preview |\n"
        table += "|---|-------------|-----------------|------------------|\n"
        ...
        for i, result in enumerate(results, 1):
            ...
            # Clean preview text and limit length
            preview = result['text'][:80].replace('|', '\\|').replace('\n', ' ').strip() + "..."
            chunk_id = result['metadata']['chunk_id']
            
            table += f"| {i} | {source_file} | {score} | {preview} |\n"
    ...
    def query_with_context(self, question: str, context: str, results: List[Dict] = None,
        ...
        # Enhanced system message for better source citation
...
- Don't make assumptions beyond what's stated in the sources"""
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                # Add the sources table to the response
                if results:
                    sources_table = self.create_sources_table(results)
                    answer += sources_table
...
def main():
    ...
    while True:
        ...
        # Send to OpenRouter with enhanced response format
        print("🧠 Generating answer with source table...")
        ...
        print(f"\n💬 Answer:\n{answer}")
        print(f"\n📊 Memory usage: {rag_result['memory_usage']}")
